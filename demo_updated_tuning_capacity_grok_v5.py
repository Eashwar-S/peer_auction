...
class MagneticFieldRouter:
    ...
    def find_route_with_magnetic_scoring(self, required_edges, verbose=False):
        ...
        # === Lookahead-based end-depot choice + post-fill pack ===
        u = current_route[-1]
        req_set_all = {tuple(sorted(e)) for e in required_edges}
        req_set_remaining = req_set_all - required_covered

        # choose best depot d* (feasible by capacity)
        d_star = self._choose_end_depot_lookahead(u, current_length, req_set_remaining)
        if d_star is None:
            # fallback: nearest depot
            d_star = min(self.depots, key=lambda d: self._dist[u].get(d, float('inf')))

        # post-fill toward d* (greedy capacity packing)
        ...
        # ---------- HELPER: endpoints of a set of required edges ----------
    def _frontier_nodes(self, req_set):
        nodes = set()
        for a, b in req_set:
            nodes.add(a); nodes.add(b)
        return nodes

    # ---------- HELPER: MST lower bound over frontier + depot ----------
    def _mst_metric_lb(self, frontier_nodes, depot):
        # Build complete graph metric over frontier ∪ {depot} using APSP distances
        ...
        for i in range(len(pts)):
            for j in range(i+1, len(pts)):
                a, b = pts[i], pts[j]
                d = self._dist[a].get(b, float('inf'))
                if d < float('inf'):
    ...
    # ---------- HELPER: single-trip greedy WITHOUT lookahead (for ghost) ----------
    def _single_trip_greedy_no_lookahead(self, required_edges, verbose=False):
        """
        A copy of your trip builder logic that simply ends at the nearest depot.
        Used only for 'ghost' evaluation to avoid recursive lookahead.
        """
        current_route = [self.start_depot]
        current_length = 0.0
        required_covered = set()
        ...
        while len(required_covered) < len(req_set_all) and iteration_count < max_iterations:
            ...
            u = current_route[-1]
            req_set_remaining = req_set_all - required_covered
            if not req_set_remaining: break
            Fu = self._dist_to_frontier(u, req_set_remaining)
            ...
            if not candidates:
                # force-move toward nearest frontier; otherwise finish
        ...
        # coverage set from route sequence
    ...
    # ---------- LOOKAHEAD: choose end depot lexicographically ----------
    def _choose_end_depot_lookahead(self, u, current_length, req_set_remaining):
        """
        Evaluate all feasible end depots reachable from node u with remaining capacity.
        Score tuple: (W_rem_after_ghost, LB_from_ghost_end, tail_len, -coverage_per_length)
        """
        D_feas = []
        for d in self.depots:
            tail = self._dist[u].get(d, float('inf'))
            if tail < float('inf') and current_length + tail <= self.capacity:
                D_feas.append((d, tail))
        if not D_feas:
            return None  # caller will fallback to nearest

        best_key, best_d = None, None
        for d, tail_len in D_feas:
            # Ghost: simulate next trip starting at d on remaining set (after current trip commitment)
            ghost_router = MagneticFieldRouter(self.graph, start_depot=d, depots=self.depots,
                                               capacity=self.capacity, alpha=self.alpha, gamma=self.gamma)
            ghost_route, ghost_len, ghost_cov = ghost_router._single_trip_greedy_no_lookahead(list(req_set_remaining))
            # Remaining after ghost
            req_after = req_set_remaining - set(ghost_cov)
            W_rem = 0.0
            for a, b in req_after:
                W_rem += self.graph[a][b]['weight']
            # Lower bound from ghost end (or d if ghost empty)
    ...
    # ---------- POST-FILL toward chosen depot to pack capacity ----------
    def _post_fill_toward_depot(self, u, current_length, d_star, req_set_remaining, current_route, required_covered):
        """
        Greedily add closest frontier edge while keeping feasibility to end at d_star.
        Modifies current_route/current_length/required_covered in place; returns new (u, current_length).
        """
        while True:
            if not req_set_remaining: break
            # pick nearest endpoint t of any remaining required edge
            best_t, best_path, best_plen, best_edge = None, None, float('inf'), None
            for (a, b) in list(req_set_remaining):
                for t, other in ((a, b), (b, a)):
                    d_ut = self._dist[u].get(t, float('inf'))
                    if d_ut < best_plen:
                        best_plen = d_ut
                        try:
                            best_path = nx.shortest_path(self.graph, u, t, weight='weight')[1:]
                            best_edge = (t, other)
                            best_t = t
                        except nx.NetworkXNoPath:
                            pass
            if best_path is None:
                break
            # cost if we take u->t path + traverse that required edge + return to d_star
            req_edge_sorted = tuple(sorted(best_edge))
            req_w = self.graph[best_edge[0]][best_edge[1]]['weight']
            return_home = self._dist[best_edge[1]].get(d_star, float('inf'))
