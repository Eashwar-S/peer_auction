...
class MagneticFieldRouter:
    ...
    def find_route_with_magnetic_scoring(self, required_edges: List[Tuple[int, int]], verbose: bool = False):
        ...
        iteration_count = 0
        current_end = self.start_depot
        prev_num_required_covered = 0
        ...
        while len(required_covered) < len(req_set_all) and iteration_count < max_iterations:
            ...
            for v in self.graph.neighbors(u):
                ...
                # print(f'current length - {current_length}, neightbor - {v}, w_uv - {w_uv}, depot after - {depot_after}, trip length - {current_length + w_uv + depot_after}, trip - {current_route}')
            ...
            if prev_num_required_covered < len(required_covered):
                

                
                opt_trip_len = self._dist[current_end][u]
                if opt_trip_len < current_length:
                    opt_trip = nx.shortest_path(self.graph, source=current_end, target=u, weight='weight')
                    current_length = opt_trip_len
                    print(f'current route - {current_route}, new route - {list(opt_trip),} req edge - {(u, v)}')
                    current_route = list(opt_trip)
                    if current_length + w_uv + depot_after <= self.capacity:
                        current_end = v
...
def plan_with_start_retargeting(
    ...
    for s_star in depots:
        ...
        trip2_route, trip2_cost_reduced, _ = router2.find_route_with_magnetic_scoring(list(req_set_remaining), verbose=True)
...
def run_intelligent_tuning_demo():
    ...
    for sce in range(66, 67):
                ...
                if ret_route is None:
                    ...
                    if state_key in _seen_backtracks:
                        ...
                        print(f"[Trip {trip_idx}] (position) start={current_start} -> end={pos_route[-1]} | "
                              f"cost={pos_cost:.2f} | covered=0 | remaining={len(req_set_remaining)}")
                        trips.append({
                            "start": current_start,
                            "end": pos_route[-1],
                            "route": pos_route,
                    ...
                    if trips:
                        ...
                        print(f"[Trip {trip_idx}] start={current_start} -> end={route_bt[-1]} | "
                              f"cost={cost_bt:.2f} | covered={len(covered_bt)} | remaining={len(req_set_remaining)}")
                        trips.append({
                            "start": current_start,
                            "end": route_bt[-1],
                            "route": route_bt,
        ...
        # assert len(req_set_remaining) == 0
