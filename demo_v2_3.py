...
START_DEPOT = 0
END_DEPOT = 5
VEHICLE_CAPACITY = 27  # Capacity that matches the terminal output
...
NEW_REQUIRED_EDGES = [(4, 5), (5, 3), (4, 6)]  # Note: (5, 3) contains end depot node 5
...
def visualize_dynamic_route_construction_enhanced(modifier, initial_route, final_route, scoring_history, successful_new_edges):
    ...
    for step_idx in range(len(scoring_history)):
        ...
        if not is_depot_step:
            ...
            # Use the ACTUAL scoring data from the algorithm execution, not recalculated
            # This ensures consistency between terminal output and visualization
            components = ['P\n(Req Edge)', 'D_mod\n(Depot)', 'R\n(Sequence)', 'C\n(Coverage)', 'E\n(Efficiency)', 'B\n(Completion)', 'U\n(Urgency)', 'T\n(Traversal)', 'G\n(Progress)']
            ...
            ax_scoring.set_title(f'Enhanced Scoring Components\nSelected Edge: {current_step["edge"]} (ACTUAL ALGORITHM DATA)', 
            ...
            # Add final score at the bottom
            final_score = score_data.get('final_score', 0)
            ax_scoring.text(0.5, -0.15, f'Final Score: {final_score:.3f}', ha='center', va='top', 
                          transform=ax_scoring.transAxes, fontsize=12, fontweight='bold',
